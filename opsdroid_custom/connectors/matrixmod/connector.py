...
CONFIG_SCHEMA = {
    ...
    Required("store"): str,
    Required("enable_encryption"): bool,

}

__all__ = ["ConnectorMatrixMod"]
...
class ConnectorMatrixMod(Connector):
    """Connector for Matrix (https://matrix.org)."""
    async def trust_devices(self, user_id: str, device_list: Optional[str] = None) -> None:
        ...
        for device_id, olm_device in self.connection.get_missing_sessions():
    ...
    async def to_device_callback(self, event):  # noqa
        ...
        logging.info("=========================")
        logging.info("to_device_callback")
        logging.info("=========================")
        try:
            client = self.connection

            if isinstance(event, KeyVerificationStart):  # first step
                logging.info("KeyVerificationStart")
                ...
                if isinstance(resp, ToDeviceError):
                    logging.info(f"accept_key_verification failed with {resp}")
                ...
                if isinstance(resp, ToDeviceError):
                    logging.info(f"to_device failed with {resp}")

            elif isinstance(event, KeyVerificationCancel):  # anytime
                logging.info("KeyVerificationCancel")
                ...
                logging.info("KeyVerificationKey")
                ...
                if isinstance(resp, ToDeviceError):
                    logging.info(f"confirm_short_auth_string failed with {resp}")


            elif isinstance(event, KeyVerificationMac):  # third step
                logging.info("KeyVerificationMac")
    ...
    def __init__(self, config, opsdroid=None):  # noqa: D107
        ...
        self.name = config.get("name", "matrixmod")  # The name of your connector
        ...
        self.store_path = config.get("store_path", str(Path(const.DEFAULT_ROOT_PATH, "matrix")))
        self.store = config.get("store", str(Path(const.DEFAULT_ROOT_PATH, "sqlite")))
    ...
    async def connect(self):
        ...
        cl_store = None
        if self._allow_encryption and self.store == "sqlite":
        ...
        if self.store == "mysql":
            ...
            config = nio.AsyncClientConfig(
                ...
                pickle_key="",
                store =  custom_store.MysqlStore if self._allow_encryption else "",
                store_sync_tokens=True,  # AGREGUE
            )
            ...
            config = nio.AsyncClientConfig(
                encryption_enabled=self._allow_encryption,
                pickle_key="",
                store_name="matrix.db" if self._allow_encryption else "",
        ...
        self.connection.add_to_device_callback(self.to_device_callback, (KeyVerificationEvent,))

        if self.access_token is not None:
            #self.connection.store = cl_store
            self.connection.restore_login(user_id=self.mxid, device_id=self.device_id, access_token=self.access_token)
    ...
    async def _parse_sync_response(self, response):
        ...
        for roomid, roomInfo in response.rooms.invite.items():
            ...
            _LOGGER.debug("events.UserInvite")
