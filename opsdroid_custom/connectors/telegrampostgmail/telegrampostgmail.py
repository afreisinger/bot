...
import aiohttp
from opsdroid.connector import Connector, register_event  # type: ignore
from opsdroid.events import File, Image, Message  # type: ignore
...
class ConnectorTelegramPostGmail(Connector):
    """A connector the the char service Telegram."""

    def __init__(self, config, opsdroid=None):  # noqa: D107
        """Init the config for the connector."""
        super().__init__(config, opsdroid=opsdroid)
        _LOGGER.debug(("Loaded Telegram Post Connector"))

        self.name = "telegrampostgmail"
        ...
        _LOGGER.info(f"self.enable:  {self.enable}")
        try:
    ...
    def handle_user_permission(self, response, user):
        ...
        if not self.whitelisted_users or user in self.whitelisted_users or user_id in self.whitelisted_users:
            return True
    ...
    async def connect(self):
        ...
        try:
            ...
            _LOGGER.error("Error connecting to telegram")
            _LOGGER.error(f"{e}")
    ...
    async def _parse_message(self, response):
        ...
        for result in response["result"]:
            ...
            if result.get("channel_post", None) or result.get("edited_channel_post", None):
                ...
                self.latest_update = result["update_id"] + 1
            elif "message" in result and "sticker" in result["message"] and "emoji" in result["message"]["sticker"]:
                self.latest_update = result["update_id"] + 1
    ...
    async def _get_messages(self):
        ...
        resp = await self.session.get(self.build_url("getUpdates"), params=data, proxy=self.proxy)
        _LOGGER.debug(resp)
        if resp.status == 409:
            _LOGGER.info(("Can't get updates because previous webhook is still active. Will try to delete webhook."))
            return
    ...
    async def get_messages_loop(self):
        ...
        while self.listening:
            try:
                ...
                _LOGGER.error("Error ing get_messages")
                _LOGGER.error(f"{e}")
                _LOGGER.error(f"{e.with_traceback}")
                pass
    ...
    async def send_message(self, message):
        ...
        if self.enable != "True":
            return
        _LOGGER.debug(("Responding with: '%s' at target: '%s'"), message.text, message.target)

        data = dict()
        data["chat_id"] = message.target
        data["text"] = message.text
        resp = await self.session.post(self.build_url("sendMessage"), data=data, proxy=self.proxy)
        if resp.status == 200:
    ...
    async def send_image(self, file_event):
        ...
        data.add_field("chat_id", str(file_event.target["id"]), content_type="multipart/form-data")
        data.add_field(
            "photo",
        ...
        resp = await self.session.post(self.build_url("sendPhoto"), data=data, proxy=self.proxy)
        if resp.status == 200:
    ...
    async def send_file(self, file_event):
        ...
        resp = await self.session.post(self.build_url("sendDocument"), data=data, proxy=self.proxy)
        if resp.status == 200:
            _LOGGER.debug(("Sent %s file successfully."), file_event.name)
        else:
            _LOGGER.debug(("Unable to send file - Status Code %s."), resp.status)
