...
class ConnectorWebsocket(Connector):
    ...
    def __init__(self, config, opsdroid=None):
        ...
        _LOGGER.debug(_("Starting Websocketmod connector."))
        self.name = config.get("name", "websocketmod")
        ...
        self.token_key = self.config.get("token_key")
    ...
    async def new_websocket_handler(self, request):
        ...
        _LOGGER.debug("new_websocket_handler")
        await self.validate_request(request)
        _LOGGER.debug("POST validate_request")
        if (
    ...
    async def validate_request(self, request):
        ...
        try:
            ...
            if "Authorization" in headers:
                ...
                _LOGGER.warning(json.dumps({'message': 'No headers found'}))
        ...
        if not auth:
            _LOGGER.warning(json.dumps({'message': 'Token not provided'}))
            return
        data = auth.split(" ")[0]
        try:
            payload = jwt.decode(data, base64.b64decode(self.token_key), algorithms=['HS256'], audience="bot")
            _LOGGER.info(payload)
            if "bot" in payload:
                if payload["bot"] != "yes":
                    raise aiohttp.web.HTTPUnauthorized()
                else:
                    _LOGGER.info(payload["preferred_username"])
                    return True
            else:
                _LOGGER.warning(json.dumps({'message': 'No claims bot'}))
                raise aiohttp.web.HTTPBadRequest()
        except jwt.ExpiredSignatureError:
            _LOGGER.warning(json.dumps({'message': 'Token expired'}))
            raise aiohttp.web.HTTPUnauthorized()
        except jwt.InvalidTokenError as e:
            _LOGGER.warning(repr(e))
            _LOGGER.warning(json.dumps({'message': 'Invalid token'}))
            raise aiohttp.web.HTTPUnauthorized()
