...
from opsdroid.events import Message, Image, File # type: ignore
...
class ConnectorTelegramPost(Connector):
    ...
    async def send_message(self, message):
        ...
        if self.enable != "True":
           return
        _LOGGER.debug(
            ("Responding with: '%s' at target: '%s'"), message.text, message.target
        )

        data = dict()
        data["chat_id"] = int(message.target)
        data["text"] = escape(message.text) # Escapa caracteres especiales
    ...
    async def send_image(self, file_event):
        ...
        #)
        data.add_field(
            "chat_id", str(file_event.target), content_type="multipart/form-data"
        )
        data.add_field(
            "photo",
